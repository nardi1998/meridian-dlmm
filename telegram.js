...
export async function notifyClose({ pair, pnlSol, pnlUsd, pnlPct, reason }) {
  if (hasActiveLiveMessage()) return;
  const pnl = pnlSol ?? pnlUsd ?? 0;
  const sign = pnl >= 0 ? "+" : "";
  await sendHTML(
    `🔒 <b>Closed</b> ${pair}\n` +
    `PnL: ${sign}◎${Math.abs(pnl).toFixed(4)} SOL (${sign}${(pnlPct ?? 0).toFixed(2)}%)` +
    (reason ? `\nReason: ${reason}` : "")
  );
}
