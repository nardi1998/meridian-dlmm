...
function getRawPoolScreeningRejectReason(pool, s) {
  ...
  const poolCreatedAt = numeric(pool?.created_at);
  ...
  if (s.maxPoolTokenAgeDeltaHours != null) {
    if (createdAt != null && poolCreatedAt != null) {
      const diffHours = Math.abs(poolCreatedAt - createdAt) / 3_600_000;
      if (diffHours > s.maxPoolTokenAgeDeltaHours) {
        return `pool age vs token age difference ${Math.round(diffHours)}h exceeds ${s.maxPoolTokenAgeDeltaHours}h`;
      }
    }
    // If either token or pool creation time is missing, do not reject by age delta.
  }
  return null;
}
