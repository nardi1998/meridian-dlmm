...
export async function getTokenInfo({ query }) {
  ...
  const results = tokens.slice(0, 5).map((t) => ({
    ...
    stats_1h: t.stats1h ? {
      ...
      liquidity_change: t.stats1h.liquidityChange?.toFixed(2),
      volume_change: t.stats1h.volumeChange?.toFixed(2),
...
export async function getMeteoraTokenInfo({ query }) {
  const url = `${METEORA_DLMM_API}/pools?query=${encodeURIComponent(query)}&timeframe=1h`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Meteora token search API error: ${res.status}`);
  const data = await res.json();
  const pools = Array.isArray(data?.data) ? data.data : [];
  if (!pools.length) return { found: false, query };

  const pool = pools.find((p) =>
    p?.token_x?.address === query || p?.token_y?.address === query
  ) || pools[0];

  const token = pool?.token_x?.address === query
    ? pool.token_x
    : pool?.token_y?.address === query
      ? pool.token_y
      : null;

  const volume1h = Number(pool?.volume?.["1h"] ?? 0);
  const feeTvl1h = Number(pool?.fee_tvl_ratio?.["1h"] ?? 0);
  ...
  return {
    ...
    query,
    results: [{
      mint: token?.address ?? query,
      name: token?.name,
      symbol: token?.symbol,
      liquidity: tvl,
      price: token?.price,
      volume_1h: volume1h,
      fee_tvl_ratio_1h: feeTvl1h,
