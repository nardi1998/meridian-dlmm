...
function shouldUseDirectLpAgentFetch() {
  return !!config.api.lpAgentRelayEnabled && !!process.env.LPAGENT_API_KEY;
}
...
export async function getPositionPnl({ pool_address, position_address }) {
  ...
  try {
    ...
    const pnlSol = roundNum(solMode ? p.pnlSol : p.pnlUsd, 4);
    return {
      pnl_sol:           pnlSol,
      pnl_usd:           pnlSol,
      pnl_pct:           roundNum(reportedPnlPct ?? derivedPnlPct ?? 0, 2),
      current_value_sol: roundNum(currentValue, 4),
      current_value_usd: roundNum(currentValue, 4),
      unclaimed_fee_sol: roundNum(unclaimedValue, 4),
      unclaimed_fee_usd: roundNum(unclaimedValue, 4),
      all_time_fees_sol: roundNum(solMode ? p.allTimeFees?.total?.sol : p.allTimeFees?.total?.usd, 4),
...
/** Authoritative closed-position metrics in SOL from Meteora closed PnL API. */
function closedMetricsFromApi(posEntry, feesSolFallback = 0) {
  if (!posEntry) return null;
  return {
    pnlSol: getClosedPnlValue(posEntry, true),
    pnlPct: getClosedPnlPct(posEntry, true),
    initialSol: safeNum(posEntry.allTimeDeposits?.total?.sol),
    finalSol: safeNum(posEntry.allTimeWithdrawals?.total?.sol),
    feesSol: safeNum(posEntry.allTimeFees?.total?.sol) || feesSolFallback,
  };
}

function buildRecordPerformancePayload(base, metrics) {
  return {
    ...base,
    pnl_sol: metrics.pnlSol,
    pnl_pct: metrics.pnlPct,
    initial_value_sol: metrics.initialSol,
    final_value_sol: metrics.finalSol,
    fees_earned_sol: metrics.feesSol,
  };
}
...
export async function getMyPositions({ force = false, silent = false, wallet_address = null } = {}) {
  ...
  const loadPositions = async () => { try {
    ...
    const lpAgentByPosition = relayLpAgentByPosition || (shouldUseDirectLpAgentFetch() ? await fetchLpAgentOpenPositions(walletAddress) : {});
          ...
          pnl_sol:            lpData
            ? Math.round((
                config.management.solMode
                  ...
                  : safeNum(lpData.pnl?.valueNative ?? lpData.pnl?.value)
              ) * 10000) / 10000
            : binData
            ? Math.round(parseFloat(binData.pnlSol ?? binData.pnlUsd ?? 0) * 10000) / 10000
            : null,
          pnl_usd:            lpData
            ? Math.round((
                config.management.solMode
...
export async function closePosition({ position_address, reason }) {
  ...
  try {
      ...
      try {
        ...
        if (tracked) {
          ...
          let closedMetrics = {
            ...
            initialSol: 0,
            finalSol: 0,
            feesSol: safeNum(tracked.total_fees_claimed_usd),
          };
          try {
            ...
            for (let attempt = 0; attempt < 6; attempt++) {
              ...
              if (res.ok) {
                ...
                const fromApi = closedMetricsFromApi(posEntry, closedMetrics.feesSol);
                if (fromApi) {
                  closedMetrics = fromApi;
                  log("close", `Relay closed PnL from API: ${closedMetrics.pnlSol.toFixed(4)} SOL (${closedMetrics.pnlPct.toFixed(2)}%)`);
                  break;
                }
              }
          ...
          await recordPerformance(buildRecordPerformancePayload({
            position: position_address,
            pool: poolAddress,
          ...
          appendDecision({
            ...
            pool_name: tracked.pool_name || poolMeta.name || poolAddress.slice(0, 8),
            position: position_address,
            summary: `Relay closed at ${closedMetrics.pnlPct.toFixed(2)}%`,
            ...
            metrics: {
              pnl_sol: closedMetrics.pnlSol,
              pnl_pct: closedMetrics.pnlPct,
              fees_sol: closedMetrics.feesSol,
              minutes_held: minutesHeld,
            },
          });
    ...
    if (tracked) {
      ...
      // Fetch closed PnL from API — authoritative source after withdrawal settles (SOL)
      let closedMetrics = {
        pnlSol: 0,
        pnlPct: 0,
        initialSol: 0,
      ...
      try {
        ...
        for (let attempt = 0; attempt < 6; attempt++) {
          ...
          if (res.ok) {
            ...
            if (posEntry) {
              const fromApi = closedMetricsFromApi(posEntry, closedMetrics.feesSol);
              if (fromApi && !shouldRejectClosedPnl(fromApi.pnlPct, reason || tracked?.close_reason)) {
                closedMetrics = fromApi;
                log("close", `Closed PnL from API: ${closedMetrics.pnlSol.toFixed(4)} SOL (${closedMetrics.pnlPct.toFixed(2)}%), withdrawn=${closedMetrics.finalSol.toFixed(4)} SOL, deposited=${closedMetrics.initialSol.toFixed(4)} SOL`);
                break;
              }
              if (fromApi) {
                log("close_warn", `Rejected unsettled closed PnL for ${position_address.slice(0, 8)} on attempt ${attempt + 1}/6: ${fromApi.pnlPct.toFixed(2)}%`);
              }
            } else {
      ...
      if (closedMetrics.finalSol === 0) {
        const cachedPos = _positionsCache?.positions?.find(p => p.position === position_address);
        if (cachedPos) {
          const pnlSol = safeNum(cachedPos.pnl_usd);
          const feesSol =
            safeNum(cachedPos.collected_fees_usd) + safeNum(cachedPos.unclaimed_fees_usd);
          const initialSol = safeNum(tracked.initial_value_usd) || safeNum(cachedPos.total_value_usd);
          closedMetrics = {
            pnlSol,
            pnlPct: cachedPos.pnl_pct ?? 0,
            feesSol,
            initialSol,
            finalSol: Math.max(0, initialSol + pnlSol - feesSol),
          };
      ...
      appendDecision({
        ...
        pool_name: tracked.pool_name || poolMeta.name || poolAddress.slice(0, 8),
        position: position_address,
        summary: `Closed at ${closedMetrics.pnlPct.toFixed(2)}%`,
        reason: reason || "agent decision",
        risks: [
          minutesOOR > 0 ? `out of range ${minutesOOR}m` : null,
          tracked.volatility != null ? `volatility ${tracked.volatility}` : null,
