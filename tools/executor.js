  ...
  try {
    ...
    if (success) {
      if (name === "swap_token" && result.tx) {
        ...
        notifyClose({ pair: result.pool_name || args.position_address?.slice(0, 8), pnlSol: result.pnl_sol ?? result.pnl_usd ?? 0, pnlPct: result.pnl_pct ?? 0, reason: args.reason ?? result.reason ?? result.close_reason }).catch(() => {});
