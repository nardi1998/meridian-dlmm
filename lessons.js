...
import { perfFeesSol, perfInitialSol, perfPnlSol, roundSol } from "./pnl-units.js";
...
export async function recordPerformance(perf) {
  ...
  const initialSol = roundSol(
    perf.initial_value_sol ?? perf.initial_value_usd ?? 0,
  );
  const finalSol = roundSol(
    perf.final_value_sol ?? perf.final_value_usd ?? 0,
  );
  const feesSol = roundSol(
    perf.fees_earned_sol ?? perf.fees_earned_usd ?? 0,
  );

  // Reject rows where USD-sized deposits were written into SOL fields by mistake.
  const suspiciousUnitMix =
    Number.isFinite(perf.amount_sol) &&
    perf.amount_sol >= 0.25 &&
    initialSol >= 20 &&
    finalSol > 0 &&
    finalSol <= perf.amount_sol * 2;

  if (suspiciousUnitMix) {
    log("lessons_warn", `Skipped suspicious performance record for ${perf.pool_name || perf.pool}: initial=${initialSol} SOL, final=${finalSol} SOL, amount_sol=${perf.amount_sol}`);
    return;
  }

  const pnlSol =
    perf.pnl_sol != null && Number.isFinite(Number(perf.pnl_sol))
      ? roundSol(perf.pnl_sol)
      : roundSol(finalSol + feesSol - initialSol);

  const pnl_pct =
    perf.pnl_pct != null && Number.isFinite(Number(perf.pnl_pct))
      ? roundSol(perf.pnl_pct, 2)
      : initialSol > 0
        ? roundSol((pnlSol / initialSol) * 100, 2)
        : 0;
  ...
  if (perf.pool) {
    ...
    recordPoolDeploy(perf.pool, {
      ...
      fee_earned_pct: initialSol > 0 ? (feesSol / initialSol) * 100 : null,
...
function derivLesson(perf) {
  const tags = [];
  const initialSol = perfInitialSol(perf);
  const feesSol = perfFeesSol(perf);
  const feeYieldPct = initialSol > 0 ? (feesSol / initialSol) * 100 : 0;
...
export function getPerformanceHistory({ hours = 24, limit = 50 } = {}) {
  ...
  const filtered = p
    ...
    .map((r) => ({
      ...
      pool: r.pool,
      strategy: r.strategy,
      pnl_sol: perfPnlSol(r),
      pnl_pct: r.pnl_pct,
      fees_earned_sol: perfFeesSol(r),
  ...
  const totalPnl = filtered.reduce((s, r) => s + perfPnlSol(r), 0);
  const wins = filtered.filter((r) => perfPnlSol(r) > 0).length;

  return {
    hours,
    count: filtered.length,
    total_pnl_sol: roundSol(totalPnl),
    total_pnl_usd: roundSol(totalPnl),
...
export function getPerformanceSummary() {
  ...
  const totalPnlSol = p.reduce((s, x) => s + perfPnlSol(x), 0);
  const avgPnlPct = p.reduce((s, x) => s + x.pnl_pct, 0) / p.length;
  const avgRangeEfficiency = p.reduce((s, x) => s + x.range_efficiency, 0) / p.length;
  const wins = p.filter((x) => perfPnlSol(x) > 0).length;

  return {
    total_positions_closed: p.length,
    total_pnl_sol: roundSol(totalPnlSol),
    total_pnl_usd: roundSol(totalPnlSol),
